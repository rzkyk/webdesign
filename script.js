document.addEventListener('DOMContentLoaded', () => {
    // ================= 1. DATABASE DATA (Jangan Diubah Kecuali Gambar) =================
    const dbGames = [
        // Pastikan nama file gambar (ml.png, dll) sesuai dengan file di folder project kamu
    ...
    const dbItems = {
        ...
        "ff": [ {n:"5 Diamond", h:1000}, {n:"70 Diamond", h:10000}, {n:"Member Mingguan", h:30000} ],
    ...
    let currentCategory = '';
    let isWaitingForResponse = false; // Untuk Chat
    ...
    // ================= 4. FUNGSI UTAMA (WINDOW ASSIGNMENT) =================
    // Kita menaruh fungsi di 'window' agar bisa dipanggil oleh onclick="..." di HTML

    window.openPage = function(kategori) {
        if (!isLoggedIn) { overlay.classList.add('active'); showToast("🔒 Login dulu bos!", "error"); return; }
        ...
        document.getElementById('pageTitle').innerHTML = kategori === 'mobile' ? "🔥 Mobile Game" : kategori === 'pc' ? "💻 PC Game" : "🎧 Gaming Gear";
        container.innerHTML = "";
            ...
            items.forEach(item => {
                container.innerHTML += `
                    <div class="card">
                        ...
                        <button class="btn-buy" onclick="addToCart('${namaGame} - ${item.n}', ${item.h}, '${imgGame}')">+ Keranjang</button>
                    </div>`;
            });
        }
        window.scrollTo(0, 0);
    ...
    window.searchGame = function() {
        ...
        if (!isLoggedIn) { showToast("🔒 Login dulu untuk cari game!", "error"); return; }
        ...
        if(hasil.length > 0) {
            ...
            container.innerHTML = "<p style='text-align:center; width:100%; color:#94a3b8;'>Game tidak ditemukan.</p>";
        }
    }
    ...
    window.addToCart = function(nama, harga, img) {
        ...
        updateCartUI();
        showToast(`🛒 ${nama} masuk keranjang!`, "success");
    }
    ...
    window.openCheckout = function() {
        if(shoppingCart.length === 0) { showToast("❌ Keranjang kosong!", "error"); return; }
    ...
    function renderGameCard(game, container) {
        container.innerHTML += `
            <div class="card" onclick="openDetail('${game.code}', '${game.nama}', '${game.img}')" style="cursor: pointer;">
                ...
                <h3>${game.nama}</h3>
                <p style="color:var(--primary); font-size:0.9rem;">Pilih Harga ➤</p>
            </div>`;
    }
    ...
    function renderCartItems() {
        ...
        if(shoppingCart.length === 0) { 
            list.innerHTML = "<p style='text-align:center; color:#94a3b8;'>Kosong...</p>"; 
    ...
    document.getElementById('loginForm').onsubmit = (e) => {
        ...
        if(email === localStorage.getItem('email') && pass === localStorage.getItem('pass')) {
            ...
            overlay.classList.remove('active'); updateAuthUI(); showToast("🚀 Login Berhasil!", "success");
        } else { showToast("❌ Salah Email/Password", "error"); }
    };
    ...
    document.getElementById('registerForm').onsubmit = (e) => {
        ...
        showToast("✅ Akun Jadi! Login kuy.", "success");
    ...
    function appendMessage(sender, text) {
        ...
        div.className = `message ${sender}`; // 'user' atau 'admin'
    ...
    function handleSend() {
        ...
        if(!isWaitingForResponse) {
            ...
            setTimeout(() => {
                let reply = "Halo! Ada yang bisa dibantu?";
                const low = txt.toLowerCase();
                if(low.includes('harga') || low.includes('bayar')) reply = "Harga sudah tertera di setiap item ya kak. Pembayaran bisa via QRIS/DANA.";
                else if(low.includes('top up') || low.includes('beli')) reply = "Silakan pilih game di halaman utama, lalu klik paket yang diinginkan.";
                else if(low.includes('lama') || low.includes('jam')) reply = "Proses otomatis 1-5 menit kak. Kalau gangguan maksimal 24 jam.";
                
                appendMessage('admin', reply);
                isWaitingForResponse = false;
            }, 1000);
        } else {
            appendMessage('admin', "Sabar ya kak, satu-satu...");
        }
    }
